...
public class Day12 implements DayInterface {
    ...
    private long generate(long nbGenerations, String state, List<Spread> spreads) {
        long furthestLeft = 0, previousFurthestLeft = 0;
        long age = 0;
        ...
        while (age < nbGenerations) {
            ...
            if (newState.toString().equals(state)) {
                ...
                long offSet = furthestLeft - previousFurthestLeft;
                furthestLeft = furthestLeft + offSet * (nbGenerations - age - 1);

                // on va directement à la fin
                age = nbGenerations;
            } else {
                state = newState.toString();
                previousFurthestLeft = furthestLeft;
                age++;
            }
        }
